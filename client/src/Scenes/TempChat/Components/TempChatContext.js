...
const TempChatProvider = props => {
    ...
    useEffect(() => {
      const getIMatched = async () => {
        ...
        let iMatched = [] 
        if (result.data.length > 0) await result.data.forEach(user => { iMatched.push(user._fields[0].properties)
        });
      ...
      const getLikesMe = async () => {
        ...
        let isLikingMe = [] 
        if (result.data.length > 0) await result.data.forEach(user => { isLikingMe.push(user._fields[0].properties)
        });
      ...
      const getILiked = async () => {
        ...
        let iLiked = [] 
        if (result.data.length > 0) await result.data.forEach(user => { iLiked.push(user._fields[0].properties)
        });
        setILiked(iLiked)
      };
      ...
      const getVisitedMe = async () => {
        ...
        let visitedMe = [] 
        if (result.data.length > 0) await result.data.forEach(user => { visitedMe.push(user._fields[0].properties)
        });
      ...
      const getIVisited = async () => {
        ...
        let iVisited = [] 
        if (result.data.length > 0) await result.data.forEach(user => { iVisited.push(user._fields[0].properties)
        });
      ...
      const getIBlocked = async () => {
        ...
        let iBlocked = [] 
        if (result.data.length > 0) await result.data.forEach(user => { iBlocked.push(user._fields[0].properties)
        });
  ...
  return (
    <TempChatContext.Provider value={[iMatched, likesMe, iLiked, visitedMe, iVisited, iBlocked]}>
      {props.children}
